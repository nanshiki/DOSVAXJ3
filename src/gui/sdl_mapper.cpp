...
//	LOG_MSG("GetKeyCode %X %X %X",keysym.scancode,keysym.sym,keysym.mod);
	if (usescancodes) {
		...
		switch (key) {
			...
			case 0x52:	// INSERT
			case 0x53:	// DELETE
			case 0x5D:  // Application key for AX
...
//					const char* geom = XGetAtomName(info.info.x11.display, desc->names->geometry);
					if(keycodes) {
						...
						if (strncmp(keycodes,"(unnamed)",9) == 0) evdev_input = true;
					}
		...
		sdlkey_map[0x7d] = SDLK_WORLD_5;//for JP layout
		sdlkey_map[0x73] = SDLK_UNDERSCORE;//for JP layout


		//Win-keys
		sdlkey_map[0xdb]=SDLK_LMETA;//not only for DOSBox shortcut, but also for AX
		sdlkey_map[0xdc]=SDLK_RMETA;//not only for DOSBox shortcut, but also for AX
