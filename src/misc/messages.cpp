...
static list<MessageBlock> LangJ;
...
static void MSG_Add_Japan(const char * _name, const char* _val) {
	/* Find the message */
	...
	/* if the message doesn't exist add it */
	LangJ.push_back(MessageBlock(_name,_val));
}
	...
	if (!mfile) {
		...
		if(japan_flag) {
			...
			if(start != NULL) {
				...
				sprintf(cname, "/usr/local/share/dosboxj/%s", start);
...
void MSG_Init(Section_prop * section) {
	...
	if (control->cmdline->FindString("-langjp",file_name,true)) {
		LoadMessageFile(file_name.c_str(), true);
	} else {
		Prop_path* pathprop = section->Get_path("languagejp");
		if(pathprop) LoadMessageFile(pathprop->realpath.c_str(), true);
	}
}
