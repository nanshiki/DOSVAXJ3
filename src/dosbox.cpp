...
static void DOSBOX_RealInit(Section * sec) {
	...
	dos.set_ax_enabled = false;
	dos.set_dosv_enabled = false;
	Bitu back_color = 0, text_color = 0xffffff;
	...
	else if (mtype == "dosv") { dos.set_dosv_enabled = true; svgaCard = SVGA_S3Trio; }
...
//	else if (mtype == "vga_pvga1a")   { svgaCard = SVGA_ParadisePVGA1A; }
	...
	if (IS_AX_ARCH || IS_J3_ARCH || IS_DOSV) {
		JFONT_Init(section);// Load DBCS fonts for AX (JEGA)
		if(IS_J3_ARCH) {
