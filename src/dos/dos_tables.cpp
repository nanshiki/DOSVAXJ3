...
void DOS_SetupTables(void) {
	...
	/* Create a fake DPB */
	dos.tables.dpb=DOS_GetMemory(((DOS_DRIVES*dos.tables.dpb_size)+15u)/16u);
	dos.tables.mediaid=RealMake(dos.tables.dpb,0x17);	//Media ID offset in DPB
	for (i=0;i<DOS_DRIVES;i++) {
		real_writeb(dos.tables.dpb,i*dos.tables.dpb_size,i);				// drive number
		real_writeb(dos.tables.dpb,i*dos.tables.dpb_size+1,i);			// unit number
		...
		mem_writew(Real2Phys(dos.tables.mediaid)+i*dos.tables.dpb_size,0);
