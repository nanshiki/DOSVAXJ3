...
void INT10_ReloadFont(void) {
	...
	case 8:
		...
		break;
	case 14:
		if ((IS_VGA_ARCH || IS_J3_ARCH) && svgaCard==SVGA_None && CurMode->mode==7) map=0x80;
		INT10_LoadFont(Real2Phys(int10.rom.font_14),false,256,0,map,14);
		break;
	case 19://for AX
		if (IS_AX_ARCH && svgaCard == SVGA_None) map = 0x80;
		INT10_LoadFont(Real2Phys(int10.rom.font_19), false, 256, 0, map, 19);
		break;
	case 16:
	default:
		if ((IS_VGA_ARCH || IS_J3_ARCH) && svgaCard==SVGA_None) map=0x80;
...
/* This should fill up certain structures inside the Video Bios Rom Area */
	...
	//for AX; real AX doesn't have 16pt font, only 8, 14 and 19.
	if (IS_AX_ARCH) {
...
void INT10_SetupRomMemoryChecksum(void) {
	if (IS_EGAVGA_ARCH || IS_J3_ARCH) { //EGA/VGA. Just to be safe
