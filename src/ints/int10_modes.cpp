...
/* J-3100 */
{ 0x064  ,M_DCGA   ,640 ,400 ,80 ,20 ,8 ,20 ,1 ,0xB8000 ,0x8000 ,100 ,449 ,80 ,400 ,0   },
...
{ 0x003  ,M_TEXT   ,640 ,480 ,80 ,25 ,8 ,19 ,8 ,0xB8000 ,0x1000 ,98  ,509 ,80 ,480 ,0	},//vtot must have been <512 because of CRT register
...
{ 0x053  ,M_EGA    ,640 ,480 ,80 ,25 ,8 ,19 ,1 ,0xA0000 ,0xA000 ,98 ,509 ,80 ,480 ,0 },
};
...
static void FinishSetMode(bool clearmem) {
	...
	if (clearmem) {
		...
		case M_EGA:	
		case M_LIN4:
			if(IS_DOSV || IS_J3_ARCH) {
				Bit16u seg = GetTextSeg();
				for(Bit16u ct = 0 ; ct < CurMode->twidth * CurMode->theight ; ct++) {
					real_writew(seg, ct * 2, 0x0720);
				}
			}
		case M_VGA:
	...
	if (IS_EGAVGA_ARCH || IS_J3_ARCH) {
		...
		if (IS_VGA_ARCH || IS_J3_ARCH) real_writeb(BIOSMEM_SEG,BIOSMEM_DCC_INDEX,0x0b);
...
//	VGA_SetMode(CurMode->type);
	...
	case MCH_DCGA:
		if(machine==MCH_DCGA) {
			mode_control=mode_control_list[0x06];
			color_select=0x3f;
		} else {
...
//	Bit8u vga_switches=real_readb(BIOSMEM_SEG,BIOSMEM_SWITCHES);
	...
	//if CRT BIOS in JP mode for AX
	if (INT10_AX_GetCRTBIOSMode() == 0x51) {
		//AX-2 functions not implemented
	...
	else if (IS_VGA_ARCH || IS_J3_ARCH) {
...
//r4467
	if (IS_AX_ARCH) {
		...
		else if(CurMode->vdispend == 480) misc_output |= 0x8;
	}
	...
	Bitu ret_start;
	if ((CurMode->special & _EGA_HALF_CLOCK) && (CurMode->type!=M_CGA2 && CurMode->type!=M_DCGA)) {
