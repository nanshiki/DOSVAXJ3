...
#define EMM_PAGEFRAME_J3	0xD000
	...
	case 0xDE:		/* VCPI Functions */
		if (!vcpi.enabled) {
			...
			case 0x06: {	/* VCPI Get Physical Address of Page in 1st MB */
				if (((Bitu)(reg_cx<<8)>=emm_pageframe) && ((Bitu)(reg_cx<<8)<emm_pageframe+0x1000)) {
...
void EMS_Init(Section* sec) {
	emm_pageframe = IS_J3_ARCH ? EMM_PAGEFRAME_J3 : EMM_PAGEFRAME;
	test = new EMS(sec);
