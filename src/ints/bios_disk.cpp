...
static bool driveProtected(Bit8u driveNum) {

	if (!imageDiskList[driveNum]->active) {
		LOG(LOG_BIOS, LOG_ERROR)("Disk %d is write-protected.", driveNum);
		last_status = 0x03;
		CALLBACK_SCF(true);
		return true;
	}
	...
	case 0x0: /* Reset disk */
		{
			...
			if (any_images && driveInactive(drivenum)) {
				...
				if ((machine==MCH_CGA) || (machine==MCH_PCJR) || machine==MCH_DCGA) {
