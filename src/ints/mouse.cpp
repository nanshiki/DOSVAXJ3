...
void Mouse_CursorMoved(float xrel,float yrel,float x,float y,bool emulate) {
	...
	if (emulate) {
		...
		if (CurMode->type == M_TEXT) {
			...
			mouse.y = y*((IS_EGAVGA_ARCH || IS_J3_ARCH)?(real_readb(BIOSMEM_SEG,BIOSMEM_NB_ROWS)+1):25)*8;
...
const char* Mouse_GetSelected(int x1, int y1, int x2, int y2, int w, int h) {
	...
	mouse_text[0]=0;
	for (int i=r1; i<=r2; i++) {
		if(GetKanjiAttr(c1, i) == 0x02) {
			ReadCharAttr(c1 - 1, i, page, &result);
			sprintf(str, "%c", result);
			strcat(mouse_text, str);
		}
		for (int j=c1; j<=c2; j++) {
		...
		if(GetKanjiAttr(c2, i) == 0x01) {
			ReadCharAttr(c2 + 1, i, page, &result);
			sprintf(str, "%c", result);
			strcat(mouse_text, str);
		}
		while (strlen(mouse_text)>0&&mouse_text[strlen(mouse_text)-1]==32) mouse_text[strlen(mouse_text)-1]=0;
		if (i<r2) strcat(mouse_text,"\r\n");
	}
	return out;
}
...
void Mouse_Select(int x1, int y1, int x2, int y2, int w, int h) {
	...
	for (int i=r1; i<=r2; i++) {
		if(GetKanjiAttr(c1, i) == 0x02) {
			real_writeb(seg,(i*c+c1-1)*2+1,real_readb(seg,(i*c+c1-1)*2+1)^119);
		}
		for (int j=c1; j<=c2; j++) {
			real_writeb(seg,(i*c+j)*2+1,real_readb(seg,(i*c+j)*2+1)^119);
		}
		if(!IS_AX_ARCH && IS_DOS_JAPANESE) {
			WriteCharTopView((i * c) * 2, c2 + 1);
		}
	}
}
