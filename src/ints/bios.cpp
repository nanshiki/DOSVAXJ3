...
static Bitu INT15_Handler(void) {
	...
	default:
		...
		reg_ah=0x86;
		CALLBACK_SCF(true);
		if ((IS_EGAVGA_ARCH) || (machine==MCH_CGA) || machine==MCH_DCGA) {
...
public:
	BIOS(Section* configuration):Module_base(configuration){
		...
		if(IS_J3_ARCH) {
			callback[11].Install(&INT60_Handler,CB_IRET,"Int 60 Bios");
			callback[11].Set_RealVec(0x60);

			callback[12].Install(&INT6F_Handler,CB_INT6F_ATOK,"Int 6F Bios");
			callback[12].Set_RealVec(0x6f);
		}
		...
		if(IS_J3_ARCH) {
			const char* const b_toshiba = "TOSHIBA";
			Bitu i;
			for(i = 0; i < strlen(b_toshiba); i++) phys_writeb(0xfe010 + i,b_toshiba[i]);
			const char* const b_j3100 = "J-3100VAXJ3";
			for(i = 0; i < strlen(b_j3100); i++) phys_writeb(0xfe000 + i,b_j3100[i]);
		}
