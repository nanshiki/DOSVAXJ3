...
　Windows 版の場合、シェルのファイルコピーや xcopy, zcopy 等でファイルの日時情
報が反映されますが、mount したドライブ上では FILMTN や FD 等でのファイルの日時
情報変更はできません。imgmount したドライブ上のファイルでは可能です。
　DOS ではファイルを読み込みモードでオープンしても日時情報等の属性変更が可能で
したが、Windows では書き込み可能なモードでオープンしないと変更できないため、こ
のような動作となります。
