...
gaijiend       外字領域の終了コードを指定します。未定義の場合 f0fc となります
               最大 200 文字分までしか使用できません。
               J-3100 モードの場合、最大 100 文字分までしか使用できません。
...
　日本語モードの場合、コマンドの応答メッセージや日付の並びが日本向けとなりま
...
　全角 16×16 フォントに Windows のフォントを使用する場合、罫線は内蔵のフォン
トデータを使用します。これは Windows フォントからデータを生成すると罫線が繋が
らないためです。
...
なります。
　半角カナや \ のデータを漢字 ROM から直接取得しているプログラムも動作します。
　スクロールはソフトウェアのみとなりますが、速度的には問題ないと思われます。
　RS-232C BIOS(int 14h) の ah=05h, 06h, 80h, 81h, 82h, 84h, 85h, 87h, f0h は
実装していません。
