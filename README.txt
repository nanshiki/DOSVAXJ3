...
mouse_wheel_key    マウスのホイール操作で入力されるキーを指定します。
                   0 入力なし、1 上下カーソルキー、2 左右カーソルキー
                   3 PgUp/PgDn、4 Ctrl+上下カーソルキー、5 Ctrl+左右カーソルキー
                   6 Ctrl+PgUp/PgDn、7 Ctrl+W/Z
                   - をつけるとイメージブートした OS では機能しません。
                   デフォルト値は -1 です。
...
dosv_graphic_mouse_cursor
               true でDOS/V 日本語モードでグラフィックのマウスカーソルを表示、
               false でテキストのマウスカーソルを表示します。
               デフォルト値は false です。
...
vtext3=sxga
vtext4=vga
dosv_graphic_mouse_cursor=false
...
　mount したドライブの容量等の取得(int 21h ah=36h, ax=7303h)は親 OS から値を取
得して返すように修正しました。ah=36h の場合 2G 超えの容量のドライブでは全体容
量は 2G、空き容量は調整した値を返します。ax=7303h の場合は 2G 超えでも正しい全
体容量と空き容量が取得できます。(ah=36h の場合の空き容量は DOSBox-X から移植)
　mount したドライブで dir した際に正しい空き容量を表示するよう修正しました。
　DOSVAXJ3 上で動作しているかどうかの判定(int 2fh ax=7476h で返値 ax=0002h)を
追加しました。

・build JP260414 (2026/4/14)
　マウスホイールオプション [sdl] mouse_wheel_key を追加しました。(DOSBox-X か
ら移植)
　DOS/V 日本語モードではマウスカーソルはテキストで表示するように修正しました。
　オプション [dosbox] dosv_graphic_mouse_cursor でマウスカーソルをグラフィック
表示にする事も可能です。切り替え用のマウスファンクションを追加しています。
　S3, ET-4000 の SVGA, XGA, SXGA の 16 色モードで int 10h ah=0ch/0dh (点の読み
書き)が機能するようにしました。
　DOSBox SVN r4494 までの更新分を適用しました。

・build JP260602 (2026/6/2)
　IRQ8～IRQ15 の割込先のデフォルトの処理を 8259 マスター・スレーブに EOI 出力後
iret するよう修正しました。DOS/V 版一太郎 4/dash 対策です。(DOSBox-X 参考)

・build JP260702 (2026/7/2)
　VGA のビットマスクレジスタの設定値によって文字表示がおかしくなっていたのを修正
しました。
　ビデオモードが 12h の場合、文字属性の bit7 が 1 で XOR で描画するよう修正しま
した。また、マウスカーソルはグラフィック表示となるよう修正しました。
　DOS/V モードの場合でも int 10h ah=02h のカーソル位置指定で bh のページ番号は
使用せず常に 0 となるよう修正しました。
